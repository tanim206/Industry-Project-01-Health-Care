...
const loginUser = async (payload: ILoginUserPayload) => {
  ...
  if (user.password === null && user.googleId !== null) {
    throw new AppError(
      httpStatus.CONFLICT,
      "User Already Has Account Registred With Google. Try to Login with Google!",
    );
  }
...
const googleLogin = async (payload: IGoogleLoginPayload) => {
  ...
  try {
    ...
    console.error("Google ID Token Verification Failed:", msg, error);
    console.error("Expected audience (GOOGLE_CLIENT_ID):", config.google_client_id);
    console.error("Received idToken length:", payload.idToken?.length, "prefix:", payload.idToken?.slice(0, 20));
    throw new AppError(
      httpStatus.UNAUTHORIZED,
      config.node_env === "development"
        ? `Google verify failed: ${msg}`
